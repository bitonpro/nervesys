...
SSH_OPTS = "-o StrictHostKeyChecking=no -o ConnectTimeout=5 -o LogLevel=ERROR -o BatchMode=yes"
...
def ssh_run(server: str, command: str, sudo: bool = True, timeout: int = 15) -> dict:
    ...
    try:
        ...
        return {"ok": r.returncode == 0, "rc": r.returncode, "stdout": r.stdout.strip(), "stderr": r.stderr.strip(), "server": server}
...
async def list_tools() -> list[Tool]:
    return [
        ...
        Tool(
            ...
            description="Full firewall status for one server",
            inputSchema={
                "type": "object",
                "properties": {"server": {"type": "string", "enum": list(SERVERS.keys())}},
                "required": ["server"],
            },
        ),
        Tool(
            name="fw_nft_rules",
            description="Show nftables INPUT rules for a server",
            inputSchema={
                "type": "object",
                "properties": {"server": {"type": "string", "enum": list(SERVERS.keys())}},
                "required": ["server"],
        ...
        Tool(
            ...
            description="CrowdSec: bouncers, decisions, collections",
            inputSchema={
                "type": "object",
                "properties": {"server": {"type": "string", "enum": list(SERVERS.keys())}},
                "required": ["server"],
        ...
        Tool(
            ...
            description="Fail2ban: jails and banned IPs",
            inputSchema={
                "type": "object",
                "properties": {"server": {"type": "string", "enum": list(SERVERS.keys())}},
                "required": ["server"],
        ...
        Tool(
            name="fw_check_port",
            description="Check if a port is open in nftables",
            inputSchema={
                "type": "object",
                "properties": {
        ...
        Tool(
            name="fw_add_port",
            description="Open a port via ufw",
            inputSchema={
                "type": "object",
                "properties": {
                    "server": {"type": "string", "enum": list(SERVERS.keys())},
        ...
        Tool(
            name="fw_remove_port",
            description="Close a port via ufw",
            inputSchema={
                "type": "object",
                "properties": {
                    "server": {"type": "string", "enum": list(SERVERS.keys())},
        ...
        Tool(
            name="fw_ban_ip",
            description="Manually ban IP across layers",
            inputSchema={
                "type": "object",
                "properties": {
                    "server": {"type": "string", "enum": list(SERVERS.keys())},
        ...
        Tool(
            name="fw_unban_ip",
            description="Unban an IP across layers",
            inputSchema={
                "type": "object",
                "properties": {
                    "server": {"type": "string", "enum": list(SERVERS.keys())},
        ...
        Tool(
            ...
            description="List all servers with firewall summary",
            inputSchema={"type": "object", "properties": {}},
        ),
    ]
...
async def call_tool(name: str, arguments: dict[str, Any]) -> list[TextContent]:
    ...
    if name == "fw_list_servers":
        ...
        for srv in SERVERS:
            r = ssh_run(srv, "sudo nft list table ip filter 2>/dev/null | grep 'policy'; systemctl is-active crowdsec fail2ban 2>/dev/null", timeout=8)
            policy = "unknown"
            crowdsec_b = "❌"
            f2b_b = "❌"
            if r.get("ok"):
                ...
                cs_active = "active" in out and "crowdsec" in out.lower()
                f2b_active = "fail2ban" in out and "active" in out
                if cs_active: crowdsec_b = "✅"
                if f2b_active: f2b_b = "✅"
            else:
                policy = f"⚠️ {r.get('error','?')}"
            lines.append(f"| {srv} | {policy} | {crowdsec_b} | {f2b_b} |")
        ...
        r2 = ssh_run(srv, "systemctl is-active crowdsec 2>/dev/null; sudo cscli decisions list 2>/dev/null | head -10; sudo cscli bouncers list 2>/dev/null | head -10")
        lines.append(f"## CrowdSec\n```\n{r2.get('stdout', r2.get('error', 'N/A'))}\n```")
        r3 = ssh_run(srv, "sudo fail2ban-client status 2>/dev/null")
        ...
        srv = arguments["server"]
        r = ssh_run(srv, "echo '---BOUNCERS---'; sudo cscli bouncers list 2>/dev/null; echo '---DECISIONS---'; sudo cscli decisions list 2>/dev/null | head -20")
        return [TextContent(type="text", text=f"```\n{r.get('stdout', r.get('error', 'N/A'))}\n```")]
        ...
        return [TextContent(type="text", text=f"❌ Port {port}/{proto} on {srv}: NOT in nftables (policy DROP → BLOCKED)")]
