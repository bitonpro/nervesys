...
# 3. Apply BitOn.Pro Sentinel Rebranding & Inject Custom UI
REPO_ROOT=$(dirname $(realpath "$0"))
if [ -d "$REPO_ROOT/aster-fork/dashboard" ]; then
    echo "🎨 Injecting custom Sentinel Dashboard UI..."
    cp -r $REPO_ROOT/aster-fork/dashboard/* mcp/dashboard/
fi
